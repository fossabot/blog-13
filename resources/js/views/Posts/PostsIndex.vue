      ...
      <card-component class="has-table has-mobile-sort-spaced" title="Posts" icon="account-multiple">
        ...
        <b-table
          ...
          <template slot-scope="props">
            ...
            <b-table-column class="has-text-centered" label="Comment Counts" field="comments_count" sortable>
              {{ props.row.comments_count }}
            </b-table-column>
            <b-table-column label="Published At">
              <b-tooltip v-bind:label="props.row.published_at">
                <small class="has-text-grey is-abbr-like" :title="props.row.published_at">{{ props.row.published_at }}</small>
              </b-tooltip>
            </b-table-column>
            <b-table-column custom-key="actions" class="is-actions-cell">
              <div class="buttons is-right">
                <router-link :to="{name:'posts.edit', params: {id: props.row.id}}" class="button is-small is-primary">
                  <b-icon icon="ccount-details" size="is-small"/>
                </router-link>
                <router-link :to="{name:'posts.edit', params: {id: props.row.id}}" class="button is-small is-primary">
...
export default {
  ...
  methods: {
    ...
    trashConfirm () {
      ...
      if (this.trashObject) {
        method = 'delete'
        url = `/api/v1/posts/${this.trashObject.id}/destroy`
